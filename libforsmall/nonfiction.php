...
WHERE c.cat_title = 'Non-Fiction' AND p.user_id = ? AND p.product_title LIKE ?";

// Main query with sorting + pagination
$sql = "SELECT p.* $baseQuery";

if ($filter === 'asc') {
  $sql .= " ORDER BY p.rent_price ASC";
} elseif ($filter === 'desc') {
  $sql .= " ORDER BY p.rent_price DESC";
}
...
$countSql = "SELECT COUNT(*) AS product_count $baseQuery";
...
<body class="nonfiction-page">
  ...
  <nav class="nav-bar">
    <div class="nav-left">
      ...
      <a href="nonfiction.php">
        <button class="nav-button active">Non-Fiction</button></a>
      <a href="classics.php">
      ...
      <a href="AllCategories.php">
        <button class="nav-button">All Categories</button></a>
    </div>
    <div class="nav-right">
  ...
  <main>
    ...
    <div class="pagination-wrapper">
      <div class="pagination">
        <?php if ($page > 1): ?>
          <a class="page-btn" href="nonfiction.php?page=<?php echo $page - 1; ?>&search=<?php echo urlencode($search); ?>&filter=<?php echo $filter; ?>"><i class="fas fa-chevron-left"></i></a>
        <?php endif; ?>

        <span class="page-info">Page <?php echo $page; ?> of <?php echo $totalPages; ?></span>

        <?php if ($page < $totalPages): ?>
          <a class="page-btn" href="nonfiction.php?page=<?php echo $page + 1; ?>&search=<?php echo urlencode($search); ?>&filter=<?php echo $filter; ?>"><i class="fas fa-chevron-right"></i></a>
        <?php endif; ?>
      </div>
    </div>
  </main>
  ...
  <style>
    .nonfiction-page .navbar .logo img {
      width: 75px;
      height: auto;
      max-height: 75px;
      object-fit: contain;
