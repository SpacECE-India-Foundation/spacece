   ...
   if($_POST['action'] =='View'){
    ...
    $order_id=  $_POST["order_id"];
      ...
      if($statement){
          ...
          <div class='col-sm-3'><input type=text class='form-control' value=".$row['grand_total']." readonly /></div> <div class='col-sm-3'><select class='form-control' id='status' name='status'> <option value='Ordered' <?php if(".$row['p_status']." == 'Ordered') echo('selected='selected'')?>Ordered</option><option value='Delivered' <?php if(".$row['p_status']." == 'Delivered') echo('selected=selected'')?>Delivered</option></select></div> </div>
