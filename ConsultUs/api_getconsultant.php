...
if($cat_name != "all"){
        // showing admin added from database
        $sql = "SELECT users.u_name,users.u_image,consultant_category.cat_name,consultant.c_office,consultant.c_from_time,consultant.c_to_time,consultant.c_language,consultant.c_fee,consultant.c_available_from,consultant.c_available_to,consultant.c_qualification FROM `consultant` join consultant_category JOIN users where consultant.c_category=consultant_category.cat_id AND consultant_category.cat_name='$cat_name' AND users.u_type='consultant';";
        $res = mysqli_query($conn,$sql);
        header('Content-Type:application/json');
