...
$stime = $_GET['stime'];
 $conmob =$_GET['conmob'];
  ...
  $sql="INSERT INTO `appointment`( `cid`, `category`, `cname`,`bid`,`com_mob`) VALUES ('$cid','$category','$name','$uid','$conmob')";
  $res= mysqli_query($conn,$sql);
  if(!$res){echo "<h3 style = 'color:white;'>sorry,unable to connect</h3>";}
      ...
      { //1.getting data into variable
        ...
        $res= mysqli_query($conn,$sql);
         ...
         if($res){
             ...
             // redirecting page
             header("location:".SITEURL.'appoint_consultant.php'."?user=$full_name");
...
<label for="atime"><b>Select A Time:</b></label>
  <input type="time" id="atime" name="atime" min="<?php echo $ctime ?>" max="<?php echo $stime?>"><br><br>
