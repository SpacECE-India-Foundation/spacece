...
-- Procedures
--
CREATE PROCEDURE `generate_due_list`()
    SQL SECURITY DEFINER
BEGIN
    SELECT I.issue_id, M.email, B.isbn, B.title
    FROM book_issue_log I 
    INNER JOIN member M ON I.member = M.username 
    INNER JOIN book B ON I.book_isbn = B.isbn
    WHERE DATEDIFF(CURRENT_DATE, I.due_date) >= 0 
        AND DATEDIFF(CURRENT_DATE, I.due_date) % 5 = 0 
        AND (I.last_reminded IS NULL OR DATEDIFF(I.last_reminded, CURRENT_DATE) <> 0);
END$$
