...
<head>
  ...
  <style>
    ...
    .course-thumb {
      ...
      width: 100%;
      object-fit: contain;
      background-color: #f8f8f8; /* placeholder bg for missing logos */
    }
...
<body>
  ...
  <section class="hero-section hero-banner-bg">
    <h5>
      <!-- new logo image change  -->
      <img class="learn_img" src="../learnonapp/img/SpacECELogo.jpg" alt="Logo" style="height: 18px; vertical-align: middle; margin-right: 6px;">
      Learn On App
    </h5>
  ...
  <div class="course-section">
    ...
    ?>
     ...
     class="course-thumb"
     alt="<?= htmlspecialchars($row1['title']) ?>">
    ...
    ?>
        <div class="card">
          <img src="<?= $row['logo'] ? htmlspecialchars($row['logo']) : 'SpacECELogo.jpg' ?>" alt="<?= htmlspecialchars($row['title']) ?>">
  ...
  <!-- New Change Video Section -->
  <section class="video-section" id="course-video">
    <h2>Videos related to <br><strong>course</strong></h2>

    <div class="video-card" 
        style="position: relative; width: 500px; height: 280px; margin: auto; border-radius: 12px; overflow: hidden; background: #000;">
      ...
      <img id="videoThumbnail"
          src="https://wallpapers.com/images/hd/pure-black-background-y8wp2r83b15xxdi6.jpg"
          alt="Video Thumbnail"
          style="width:100%; height:100%; object-fit: cover; border-radius:12px;">

      <!-- Like / Dislike -->
      <div style="position: absolute; top: 10px; left: 10px; color: orange; font-size: 1.5rem;">
        <i class="bi bi-hand-thumbs-up" id="likeBtn"></i>
        <i class="bi bi-hand-thumbs-down" id="dislikeBtn" style="margin-left:10px;"></i>
      </div>

      <!-- Share -->
      <i class="bi bi-share-fill" id="shareBtn"
        style="position:absolute; top:10px; right:10px; color:orange; font-size:1.5rem;"></i>

      <!-- Play/Pause -->
      <button id="playBtn"
              style="position:absolute; top:50%; left:50%; transform:translate(-50%,-50%);
                    background:white; border:none; border-radius:50%; width:60px; height:60px;">
        <i id="playIcon" class="bi bi-play-fill" style="font-size:2rem; color:orange;"></i>
      </button>

      <!-- Dots -->
      <div style="position: absolute; bottom: 10px; left: 50%; transform: translateX(-50%); display:flex; gap:5px;">
        <div style="width:8px; height:8px; background:orange; border-radius:50%;"></div>
        <div style="width:8px; height:8px; background:gray; border-radius:50%;"></div>
        <div style="width:8px; height:8px; background:gray; border-radius:50%;"></div>
      </div>
    </div>
  </section>
  ...
  <script>
    ...
    playBtn.addEventListener("click", () => {
      if (video.style.display === "none") {
        ...
        video.play();
        playIcon.className = "bi bi-pause-fill"; // change to pause
      } else {
        // Toggle play/pause
        if (video.paused) {
          video.play();
    ...
    <?php while ($row = mysqli_fetch_assoc($result)) { ?>
      <div class="course-box">
        <div class="course-header" onclick="toggleDetails(<?= $row['id'] ?>)">
          ...
          <div>
            <h5 class="course-title">course-headerx
    ...
    <script>
      $(document).ready(function() {
        ...
        function handleSubscriptionResponse(data) {
          // switch (data.trim()) {
          //   case 'Error':
          //     swal("Error!", "Somthing Went Rong Please Again Latter", "error");
          //     break;
          //   case 'Success':
          ...
          switch (data.trim()) {
            case 'Already':
              swal("Info!", "You have already subscribed to this site!", "info");
              break;
            case 'Success':
