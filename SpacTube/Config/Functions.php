...
// password: Avani@1234 --- IGNORE ---
    ...
    $conn = mysqli_connect("localhost", "root", "Avani@1234", "spactube");
    return $conn;
}
    ...
    //     /// echo $select_assoc;
    //     $select_assoc_query = mysqli_query($this->conn, $select_assoc);
    //     if (mysqli_num_rows($select_assoc_query) > 0) {  //  fixed >0 instead of >1
    //         return true;
    //     }
    //     return false;
    // }
    ...
    public function checkSubscription($table, $email) {
        $select_assoc = "SELECT * FROM $table WHERE email='$email'";
        $select_assoc_query = mysqli_query($this->conn, $select_assoc);
        return (mysqli_num_rows($select_assoc_query) > 0);
    }
...
if (isset($_POST['subscribe']) && isset($_POST['email'])) {
    ...
    if ($functions->checkSubscription("subscription", $email)) {
        echo "Error"; // already subscribed
        exit;
    }

    // 3. Insert new subscriber
    $insert = $functions->insert("subscription", [
        "email" => $email,
