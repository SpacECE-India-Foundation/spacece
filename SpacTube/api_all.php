...
$today = date('Y-m-d');
       ...
       // $sql2="SELECT * FROM `videos`, `tbl_recents` WHERE `tbl_recents`.`v_id`=`videos`.`v_id` and `tbl_recents`.`vr_date`='$today' AND `tbl_recents`.`u_no`='$uid' ORDER BY `videos`.`v_id`";;
        ...
        if($res2){
           ...
           echo json_encode(['status'=>'true','data'=>$arr, 'data_recent'=> $arr2 ,'data_trending'=> $arr3,'result'=>'found']);   // displaying data
...
$today = date('Y-m-d');
        ...
        if($res2){
           ...
           echo json_encode(['status'=>'true','data'=>$arr, 'data_recent'=> $arr2 ,'result'=>'found']);   // displaying data
